...
import { SerieAClubStats } from '../../../Api/api';
...
const useClubStats = () => {
    ...
    const loadValue = async () => {
        setLoading(true);
        try {
            const serieAClubs = await SerieAClubStats();

            console.log(serieAClubs.data);
            setserieAClubs(serieAClubs.data);

            // let res = await fetch('http://127.0.0.1:8000/serieAClub/')
    ...
    return { loading, serieAClubs };
}

const Value = () => {
    const { loading, serieAClubs } = useClubStats();
        ...
        loading ? (
            ...
            <Container>
                ...
                <Content>
                    ...
                    {serieAClubs && serieAClubs.length > 0 && (
                        <>
                            <LeagueDiv>
                            ...
                            <Item>
                                <SLink to="/serieAPStats"><SectionTitle>Serie A</SectionTitle></SLink>
                                <SectionHr></SectionHr>
                                <Table>
                                    <thead>
                                        <tr>
